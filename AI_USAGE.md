...
The switch had one more step in it. Once Gemini was actually being called, the newest Flash model answered 503, high demand, on every attempt. The pipeline degraded exactly as section 7 intends, scoring the deterministic criterion and marking the rest "not assessed", so the demo stayed usable while the judged half was unavailable. Moving `GEMINI_MODEL` one generation back fixed it. I would rather the deployed version grade with a slightly older model than look broken to anyone who opens it.
